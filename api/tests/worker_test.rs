...
async fn process_events_batch_encrypted_messages() {
    ...
    let alice_session: String = sqlx::query_scalar(
        "INSERT INTO sessions (id, user_username) VALUES ($1, 'alice_worker') RETURNING id",
    )
    .bind(Uuid::new_v4())
    .fetch_one(&pool)
    .await
    ...
    let ciphertexts: Vec<(String, i32, String)> = sqlx::query_as(
...
async fn process_events_batch_mixed() {
    ...
    let bob_session: String = sqlx::query_scalar(
        "INSERT INTO sessions (id, user_username) VALUES ($1, 'bob_mixed') RETURNING id",
    )
    .bind(Uuid::new_v4())
    .fetch_one(&pool)
    .await
