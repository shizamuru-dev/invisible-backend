...
async fn main() {
    ...
    let bucket = Bucket::new("uploads", region, credentials).unwrap().with_path_style();
    
    let mut custom_headers = HeaderMap::new();
    custom_headers.insert("content-type", "image/jpeg".parse().unwrap());
    
    let url = bucket.presign_put("/test_with_header.jpg", 3600, Some(custom_headers), None).await.unwrap();
